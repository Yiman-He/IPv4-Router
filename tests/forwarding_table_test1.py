...
def ip_forwarding_tests():
    ...
    s.expect(PacketInputTimeoutEvent(1), 'wait 1s.')
    s.expect(PacketOutputEvent(intfs[1].name, pkt21, display=Arp), "Router should send ARP request for {} on {} for the {}th time".format(hosts1[2].ip_addr, intfs[1].name, 3))
    s.expect(PacketInputTimeoutEvent(1), 'wait 1s.')
